...
![](../../../assets/images/Laplace-transform-concept.png) 
...
def fopdt(Kp, tau, theta):
    ...
    num,den = ctrl.pade(theta, 0)
    delay = ctrl.tf(num, den)
...
ax.plot(t_by_tau, y, color='#65489d')

ax.plot([0, t_632], [y_632, y_632], color='#0d5e5e', linestyle='--')
ax.plot([t_632, t_632], [0, y_632], color='#0d5e5e', linestyle='--')

ax.plot([0, t_by_tau_5], [y_t_by_tau_5, y_t_by_tau_5], color='#0d5e5e', linestyle='--')
ax.plot([t_by_tau_5, t_by_tau_5], [0, y_t_by_tau_5], color='#0d5e5e', linestyle='--')

ax.set_xlabel('$\\frac{t}{\\tau}$')
ax.set_ylabel('Normalized Response $\\left[\\frac{h(t)}{h(\\infty)}\\right]$')
ax.set_xlim([0, 8])
ax.set_ylim([0, 1.2])

ax.text(1.1, 0.632, '0.632')

# Save as SVG
fig.tight_layout()
filename = "./fopdt-normalized-response.svg"
fig.savefig(filename, format="svg")

plt.close(fig)
```
![](./fopdt-normalized-response.svg){width=90%}
...
ax.plot(t_by_tau, yd1)
ax.plot(t_by_tau, yd2)
ax.plot(t_by_tau, yd3)
ax.plot(t_by_tau, yd4)

# Text + arrow
ax.text(1.4, 0.2, '$\\tau_d = \\tau$')
ax.arrow(1.4, 0.2, -0.3, -0.15, color='black', width=0.01, length_includes_head=True)

ax.text(8.2, 1, '$k = 1$')
ax.text(8.2, 2.4, '$k = 2.5$')
ax.text(8.2, 4, '$k = 4$')

# Tau labels
ax.text(6.9, 3.8, '$\\tau = 1$')
ax.text(6.9, 1.05, '$\\tau = 1$')
ax.text(6.9, 2.55, '$\\tau = 2$')
ax.text(6.9, 2.25, '$\\tau = 1$')

# Save to SVG
fig.tight_layout()
filename = "./response-of-first-order-system.svg"
fig.savefig(filename, format="svg")

plt.close(fig)

```
![](./response-of-first-order-system.svg)
...
ax.plot(t, y5,  label=r'$\tau = 5$')
ax.plot(t, y10, label=r'$\tau = 10$')
ax.plot(t, y20, label=r'$\tau = 20$')

ax.axhline(1.0, color='#535353', linestyle='--')
...
fig.tight_layout()
fig.savefig("tau-comparison.svg", format="svg")
plt.close(fig)
```
![](./tau-comparison.svg)
...
ax.plot(t, y_z2,   label=r'$\xi = 2$')
ax.plot(t, y_z,    label=r'$\xi = 1$')
ax.plot(t, y_zp5,  label=r'$\xi = 0.5$')
ax.plot(t, y_zp25, label=r'$\xi = 0.25$')
# ax.plot(t, y_zm1, label=r'$\xi = -0.1$')
...
# Save to SVG instead of showing
fig.tight_layout()
fig.savefig("damping-ratio-responses.svg", format="svg")
plt.close(fig)

```
![](./damping-ratio-responses.svg)
...
::: {.fragment}
- The function $C[\cdot]$ constitutes the control law.
:::
...
# Plot the responses
ax.plot(t_ol,y_open_loop)
ax.plot(t_cl_p5,y_setpoint_p5)
ax.plot(t_cl_1,y_setpoint_1)
ax.plot(t_cl_4,y_setpoint_4)
...
ax.text(31, 1-0.01, 'Open loop response')
ax.text(31, 0.5-0.01, '$K_p = 0.5$')
ax.text(31, 0.33-0.01,'$K_p = 1$')
ax.text(31, 0.1-0.01, '$K_p = 4$')

fig.tight_layout()
fig.savefig("kp-responses.svg", format="svg")
plt.close(fig)
...
ax.plot(t_sp_2,y_setpoint_2)
ax.plot(t_sp_10,y_setpoint_10)
ax.plot(t_sp_30, y_setpoint_30)
...
ax.text(4, 1.67, "$\\tau_I = 2$")
ax.text(10, 1.37, "$\\tau_I = 10$")
ax.text(20, 1.15,"$\\tau_I = 30$")

fig.tight_layout()
fig.savefig("taui-responses.svg", format="svg")
plt.close(fig)
```
![](./taui-responses.svg)
...
ax.plot(t_cl_d2,y_d2)
ax.plot(t_cl_d10,y_d10)
ax.plot(t_cl_d30, y_d30)
...
ax.text(10, 0.125, "$\\tau_d = 2$")
ax.text(20, 0.085, "$\\tau_d = 10$")
ax.text(30, 0.06,"$\\tau_d = 30$")

fig.tight_layout()
fig.savefig("taud-responses.svg", format="svg")
plt.close(fig)
```
![](./taud-responses.svg)
...
# Plot the responses
fig, ax = plt.subplots()
ax.plot(t,y_p,  label='P')
ax.plot(t,y_pi,  label='PI')
ax.plot(t, y_pid,  label='PID')
...
fig.tight_layout()
fig.savefig("mixing-tank-setpoint-change.svg", format="svg")
plt.close(fig)
```
![](./mixing-tank-setpoint-change.svg)
...
fig.tight_layout()
fig.savefig("mixing-tank-disturbance-rejection.svg", format="svg")
plt.close(fig)
```
![](./mixing-tank-disturbance-rejection.svg)
