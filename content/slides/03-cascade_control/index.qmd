...
### Process models
- Open loop stable
  $$G_p = \frac{K_p exp(-\theta s)}{\tau s + 1}$$

- Integrating
  $$G_p = \frac{K_p exp(-\theta s)}{s}$$
...
- Primary process
  $$G_{p1} = \frac{K_{p1} exp(-\theta_1 s)}{\tau_1 s + 1}$$

- Secondary process
  $$G_{p2} = \frac{K_{p2} exp(-\theta_2 s)}{\tau_2 s + 1}$$
...
- Characteristic equation
  ...
  $$ 1 + \frac{K_{C2} K_{P2} exp(-\theta_2 s)}{\tau_2 s + 1} = 0 $$
:::

:::{.column width=50%}
- Let Loop gain $K_{L2} = K_{C2} K_{P2}$

- Delay:  $exp(-\theta_2 s) \approxeq 1 - \theta_2 s$
...
- Characteristic equation (CE): 
  ...
  1 + \frac{K_{c1} K_0 K_{p1} (\tau_I s + 1) exp(-\theta_t s)}
           { \tau_I s (\tau_1 s + 1)} = 0
  $$
...
- Let loop gain $K_{L1} = K_{C1} K_0 K_{p1}$ and $exp(-\theta_t s) \approxeq 1 - \theta_t s$
