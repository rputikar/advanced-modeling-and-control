...
- Component balance
  ...
  $$ \frac{d(V \rho x)}{dt} = w_1 x_1 + w_2 x_2 - w x $$ {#eq-blend_component_bal}

It is possible to further simplify the system of two differential equations to 

$$ \frac{dV}{dt} = \frac{1}{\rho}\left(w_1 + w_2 - w \right) $$ {#eq-blend_1}
...
# In newer versions, the control.root_locus function returns a single value, a
# tuple containing the root locations and gain values, instead of two separate
# values.
root_locus_data = control.root_locus(sys)
rlist = root_locus_data[0,0]
klist = root_locus_data[0,1]
